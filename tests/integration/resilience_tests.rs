...
async fn test_rate_limiting_rejects_excessive_http_requests_to_readyz() -> Result<()> {
        ...
        match res {
            Ok(response) => {
                if response.status() == StatusCode::OK {
                    ...
                    warn!(
                        "Requisição {} obteve status inesperado: {}",
                        i + 1,
                        response.status()
                    );
                }
    ...
    info!("Requisições bem-sucedidas: {}, Requisições limitadas/timeout: {}", successful_requests, rate_limited_count);
...
async fn test_websocket_inactive_client_is_disconnected_by_server() -> Result<()> {
    ...
    .await?;
    info!(
        "Teste: Conectando WebSocket e mantendo inativo. URL: {}",
        test_env.mcp_ws_url
    );
...
async fn test_server_recovers_after_typedb_temporary_outage() -> Result<()> {
    ...
    assert!(list_dbs_result_before.is_ok(), "list_databases falhou antes da simulação de falha: {:?}", list_dbs_result_before.err());
    ...
    let readyz_down_result = wait_for_mcp_server_ready_from_test_env(&test_env, Duration::from_secs(30)).await;
    ...
    let readyz_url = format!("{}{}", test_env.mcp_http_base_url, constants::MCP_SERVER_DEFAULT_READYZ_PATH);
    let direct_check_client = reqwest::Client::builder().danger_accept_invalid_certs(test_env.is_mcp_server_tls).build()?;
    let resp_down = direct_check_client.get(&readyz_url).send().await?.json::<serde_json::Value>().await?;
    assert_eq!(resp_down.get("status").and_then(|s| s.as_str()), Some("DOWN"));
    assert_eq!(resp_down.get("components").and_then(|c|c.get("typedb")).and_then(|s|s.as_str()), Some("DOWN"));
    ...
    assert!(list_dbs_result_during_outage.is_err(), "list_databases deveria falhar enquanto TypeDB está parado.");
    info!("'list_databases' falhou como esperado durante a indisponibilidade do TypeDB: {:?}", list_dbs_result_during_outage.err());
    ...
    test_env.docker_env.wait_for_service_healthy(constants::TYPEDB_SERVICE_NAME, constants::DEFAULT_TYPEDB_READY_TIMEOUT).await?;
    ...
    info!("Teste: Verificando /readyz do MCP Server após recuperação do TypeDB.");
     let readyz_up_again = wait_for_mcp_server_ready_from_test_env(&test_env, Duration::from_secs(45))
        .await
        .context("MCP Server não voltou ao estado UP após recuperação do TypeDB")?;
    assert_eq!(readyz_up_again.get("components").and_then(|c|c.get("typedb")).and_then(|s|s.as_str()), Some("UP"));

    let list_dbs_text_after_recovery = match client.call_tool("list_databases", None).await {
        Ok(res) => {
            info!("'list_databases' bem-sucedido com cliente existente após recuperação do TypeDB.");
        ...
        Err(McpClientError::WebSocket(_)) | Err(McpClientError::ConnectionClosed) => {
            ...
            let mut new_client = test_env.mcp_client_with_auth(Some(
                 "typedb:manage_databases typedb:read_data typedb:admin_databases",
            )).await?;
            let res = new_client.call_tool("list_databases", None).await
                .context("Falha no 'list_databases' com novo cliente após recuperação do TypeDB.")?;
            client = new_client;
            get_text_from_call_result(res)
        }
        Err(e) => {
            // Convert McpClientError to anyhow::Error before using context
            return Err(anyhow::Error::new(e)).context("Falha inesperada no 'list_databases' após recuperação do TypeDB.");
        }
    };

    let dbs_after: Vec<String> = serde_json::from_str(&list_dbs_text_after_recovery)?;
    assert!(dbs_after.contains(&db_name.to_string()), "Banco de dados não listado após recuperação do TypeDB.");
