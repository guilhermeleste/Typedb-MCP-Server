...
impl JwksCache {
    ...
    pub async fn refresh_keys(&self) -> Result<(), AuthErrorDetail> {
        ...
        if !response.status().is_success() {
            ...
            let err_body =
                response.text().await.unwrap_or_else(|_| String::from("<corpo do erro ilegível>"));
    ...
    pub async fn get_decoding_key_for_kid(
        ...
        let mut needs_refresh =
            self.last_updated.read().await.map_or(true, |last_update_time| {
        ...
        if needs_initial_fetch || needs_refresh {
            if let Err(e) = self.refresh_keys().await {
                if self.keys.read().await.is_none() {
                    tracing::error!(
                        "Falha crítica ao buscar JWKS pela primeira vez ou cache vazio: {}",
                        e
                    );
                    return Err(e);
                }
                tracing::warn!(
                    "Falha ao atualizar JWKS (tentando usar cache antigo se disponível): {}",
                    e
                );
            }
        }
            ...
            Some(jwk_set) => {
                match jwk_set.find(kid) {
                    Some(jwk) => DecodingKey::from_jwk(jwk)
                        .map_err(|e| {
                            AuthErrorDetail::TokenInvalid(format!(
                                "JWK para kid '{}' (pós-refresh) inválido: {}",
                                kid, e
                            ))
                        })
                        .map(Some),
...
mod tests {
    ...
    use jsonwebtoken::jwk::{
        ...
        Jwk, JwkSet, KeyAlgorithm, PublicKeyUse as JwkPublicKeyUse, RSAKeyParameters, RSAKeyType,
    };
    use jsonwebtoken::{encode, Algorithm, EncodingKey, Header as JwtHeader};
    use std::time::{Duration, SystemTime, UNIX_EPOCH};
    use tower::ServiceExt;
    use wiremock::matchers::{method, path};
    use wiremock::{Mock, MockServer, ResponseTemplate}; // Para app.oneshot()
...
-----END PRIVATE KEY-----"; // << SUBSTITUA POR UMA SEGUNDA CHAVE PRIVADA PEM REAL E COMPLETA DIFERENTE
    ...
    fn generate_test_jwt_for_src_auth(
        ...
        encode(&header, claims, &encoding_key)
            .expect("Falha ao gerar JWT de teste para src/auth.rs")
    }
    ...
    fn test_oauth_config_for_src_auth(jwks_uri: String, enabled: bool) -> config::OAuth {
        config::OAuth {
            ...
            jwks_refresh_interval_raw: Some("300s".to_string()), // Corresponds to 300 seconds
    ...
    async fn test_oauth_middleware_valid_token() {
        ...
        assert!(
            jwks_cache.is_cache_ever_populated().await,
            "Cache JWKS não populado após refresh forçado."
        );
    ...
    async fn test_oauth_middleware_disabled() {
        let _ = tracing_subscriber::fmt().with_test_writer().try_init();
        let oauth_config =
            Arc::new(test_oauth_config_for_src_auth("http://dummy/jwks".to_string(), false));
        let http_client = reqwest::Client::new();
        let jwks_cache =
            Arc::new(test_jwks_cache_for_src_auth("http://dummy/jwks".to_string(), http_client));
    ...
    async fn test_oauth_middleware_invalid_token_bad_signature() {
        ...
        let token_bad_sig = generate_test_jwt_for_src_auth(
            &claims,
            Algorithm::RS256,
            TEST_UNIT_KID_RS256,
            DIFFERENT_TEST_UNIT_RSA_PRIVATE_KEY_PEM,
        );
    ...
    async fn test_jwks_cache_refresh_and_get_key() {
        ...
        match decoding_key_result {
            Ok(Some(_key)) => {}
            Ok(None) => panic!(
                "Chave de decodificação não encontrada para kid conhecido '{}'",
                TEST_UNIT_KID_RS256
            ),
            Err(e) => panic!(
                "Erro ao obter chave de decodificação para kid '{}': {:?}",
                TEST_UNIT_KID_RS256, e
            ),
        }
